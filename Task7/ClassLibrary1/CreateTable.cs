...
namespace Excels
{
    ...
    public class CreateTable
    {
        ...
        /// <summary>
        /// Create table by specialty.
        /// </summary>
        /// <param name="path">Path to file.</param>
        public void CreateTableBySpecialty(string path)
        {
            using (ExcelMain excel = new ExcelMain())
            {
                if (excel.Open(path))
                {
                    ...
                    excel.Set(1, "A", "Date start session");
                    excel.Set(1, "B", "Date finish session");
                    excel.Set(1, "C", "Specialty");
                    excel.Set(1, "D", "Avrage score");

                    var specialites = from t in groups
                                      select t.Specialty;
                    specialites = specialites.Distinct<string>();
                    ...
                    {
                        excel.Set(row, "A", session.DateStart);
                        excel.Set(row, "B", session.DateFinish);
                        foreach (var specialty in specialites)
                        {
                            if (specialty != null)
                            {
                                var ourStudent = from t in rows
                                                 where t.Specialty == specialty
                                                 && t.StartSession == session.DateStart
                                                 && t.FinishSession == session.DateFinish
                                                 select t;

                                if (ourStudent != null)
                                {
                                    double average = CountAveregeScore(ourStudent);

                                    excel.Set(row, "C", specialty);
                                    excel.Set(row, "D", average);
        ...
        /// <summary>
        /// Create table by examiner
        /// </summary>
        /// <param name="path">Path to file.</param>
        public void CreateTableByExaminer(string path)
        {
            using (ExcelMain excel = new ExcelMain())
            {
                if (excel.Open(path))
                {
                    ...
                    excel.Set(1, "A", "Date start session");
                    excel.Set(1, "B", "Date finish session");
                    excel.Set(1, "C", "Examiner");
                    excel.Set(1, "D", "Avrage score");

                    var examiners = from t in exams
                                    select t.Examiner;
                    examiners = examiners.Distinct<string>();
        ...
        /// <summary>
        /// Create table by subjects.
        /// </summary>
        /// <param name="path">Path to file.</param>
        public void CreateTableBySubjects(string path)
        {
            using (ExcelMain excel = new ExcelMain())
            {
                if (excel.Open(path))
                {
                    ...
                    excel.Set(1, "A", "Date start session");
                    excel.Set(1, "B", "Date finish session");
                    excel.Set(1, "C", "Subjects");
                    excel.Set(1, "D", "Avrage score");

                    var subjects = from t in exams
                                    select t.Title;
                    subjects = subjects.Distinct<string>();
        ...
        /// Count average score.
        /// </summary>
        /// <param name="lists">List with rows.</param>
        /// <returns>Average score.</returns>
        private double CountAveregeScore(IEnumerable<Rows> lists)
        {
            int average = 0;
